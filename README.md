...
Every folder stores its state in the S3 bucket `devops-90s-dev` with a
...
The current bucket `devops-90s-dev` is hardcoded in **every**
...
grep -rl "devops-90s-dev" . --include=provider.tf \
  | xargs sed -i 's/remote-state-90s-dev/YOUR-BUCKET-NAME/g'
```

### 2. Domain name + Route53 hosted zone
The domain `daws-90s.space` and hosted-zone id `Z0615620336P2Y2J3JXF0` are used for
all DNS records and the ACM certificate. Replace both with your own registered
domain and its zone id. They appear as:

- `variable "domain_name"` (default `daws90s.space`) — in `40-databases`,
  `60-catalogue`, `70-acm`, `80-frontend-alb`, `90-components` `variables.tf`.
- `variable "zone_id"` (default `Z0615620336P2Y2J3JXF0`) — same folders.
- A few **hardcoded** `daws90s.space` strings inside `main.tf` of `50-backend-alb`
...
grep -rl "daws90s.space" . --include=*.tf \
...
> `roboshop-dev.daws90s.space` → `roboshop-dev.your-domain.com`.
...
- Reusable modules live in separate GitHub repos under the
  [`sriteja1204`](https://github.com/Sriteja1204) org (VPC, SG, and component modules).
